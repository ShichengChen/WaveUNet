...
from readchinese import Dataset, Testset, RandomCrop, ToTensor,Valtset
...
if(USEBOARD):writer = SummaryWriter(log_dir='../conditioned-wavenet/runs/'+str(current_time)+',mulaw,6batch,chinesesong',comment="uwavenet")
...
training_set = Dataset(np.arange(6), 'chinesesongs/',transform=None)
test_set = Testset(np.arange(6), 'chinesesongs/')
validation_set =Valtset(np.arange(6), 'chinesesongs/')


worker_init_fn = lambda worker_id: np.random.seed(np.random.get_state()[1][0] + worker_id)
loadtr = data.DataLoader(training_set, batch_size=6,shuffle=True,num_workers=10,worker_init_fn=worker_init_fn)
loadtest = data.DataLoader(test_set,batch_size=1,num_workers=10)
loadval = data.DataLoader(validation_set,batch_size=6,num_workers=10,worker_init_fn=worker_init_fn)
# In[6]:
...
def train(epoch):  # training data, the audio except for last 15 seconds
    for iloader,xtrain, ytrain in loadtr:
        ...
        for i, ind in enumerate(idx):
            data = xtrain[:, :, ind:ind + sampleSize//batchSize].to(device)
            target = ytrain[:, :,ind:ind + sampleSize//batchSize].to(device)
            optimizer.zero_grad()
            output = model(data)
