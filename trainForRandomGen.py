...
resumefile = 'model/saveForTransfer22+0.1'  # name of checkpoint
...
if(USEBOARD):writer = SummaryWriter(log_dir='runs/'+str(current_time)+'cc+muForTransfer',comment="uwavenet")
...
def train(epoch):  # training data, the audio except for last 15 seconds
    ...
    if epoch % 5 == 0 and saveFile == True:
        ...
        state = {'epoch': epoch,
                 ...
                 'iteration': iteration}
        torch.save(state, 'model/saveForTransfer22+0.1+50')
        print('write finish')
...
for epoch in range(100000):
    ...
    if (epoch+start_epoch) % 50 == 0 and (epoch+start_epoch) > 0: test(epoch+start_epoch)