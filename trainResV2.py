...
resumefile = 'model/SERes14Unet'  # name of checkpoint
...
if(USEBOARD):writer = SummaryWriter(log_dir='runs/'+str(current_time)+'cc+mu,SERes14Unet,30songs,4seconds')
...
def train(epoch):  # training data, the audio except for last 15 seconds
    ...
    for iloader,xtrain, ytrain in loadtr:
        ...
        cnt, aveloss = 0, 0
        print('train and load time or just loadding time:',time.time()-start_time)
        start_time = time.time()
        model.train()
